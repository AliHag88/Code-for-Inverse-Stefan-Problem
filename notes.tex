...
element $J_s$ above, which amounts to a projection step.
...
Section~\ref{sec:code-listing-precond}.

However, this formulation does not take into account one condition
...
\end{gather*}
Since ${\Delta s}(0)$ is not actually arbitrary, we \emph{cannot} assume in
general that $J_s'(0)=0$.
Given that $s(0)=s_0$ is fixed, it may instead make sense to impose the
condition $J_s(0)=0$.
